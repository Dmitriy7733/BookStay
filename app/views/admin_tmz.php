...
// Проверка на аутентификацию и роль администратора
...
include 'app/includes/header_admin_tmz.php';
    ...
    <div class="row">
        ...
        foreach ($categoryGroups as $category) {
            ...
            if (!empty($category['subcategories'])) {
                //print_r($category['subcategories']); //для отладки
                foreach ($category['subcategories'] as $subcategory) {
                    echo "<a class='dropdown-item' href='#' data-toggle='modal' data-target='#instructionsListModal' data-subcategory-id='{$subcategory['id']}' data-subcategory-name='{$subcategory['name']}'>{$subcategory['name']}</a>";
                }
            } else {
...
<!-- Раздел "Памятка" -->
    <div id="memo_tmz">
        ...
        <p>Данный веб-сайт является закрытым, доступным только для зарегистрированных сотрудников ОАО "ТМЗ им. В.В.Воровского". 
            Вы являетесь администратором. Вы можете удалять/добавлять: категории, подкатегории, документы, редакторов, пользователей.
            ...
            который появляется при клике (выборе) подкатегории. Добавить/удалить пользователей Вы можете через кнопку "Добавить/удалить".
...
<div class="modal fade" id="userModal" tabindex="-1" role="dialog" aria-labelledby="userModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="userModalLabel">Список пользователей</h5>
            ...
            <div class="modal-body">
                <table class="table">
                    ...
                    <tbody id="userTableBody">
                        <!-- Данные пользователей будут загружены сюда с помощью JavaScript -->
                    </tbody>
                </table>
                ...
                <form id="addUserForm">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="role">
                            <option value="editor_tmz">Редактор</option>
                            <option value="user_tmz">Пользователь</option>
                        </select>
                    </div>
...
<script>
    ...
    // Обновляем содержимое iframe перед открытием модального окна
...
$('#userModal').on('show.bs.modal', loadUsers);

function loadUsers() {
    const userTableBody = $('#userTableBody');
    userTableBody.empty(); // Очистить таблицу перед загрузкой

    $.ajax({
        url: '../index.php?page=getTmzUsers', // URL запроса
            ...
            users.forEach(user => {
                const row = `
                    <tr>
                        <td>${user.username}</td>
                        <td>${user.role}</td>
                        <td><button class="btn btn-danger" onclick="deleteUser('${user.username}')">Удалить</button></td>
                    </tr>
                `;
                userTableBody.append(row); // Добавить строку в таблицу
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('Ошибка загрузки пользователей:', textStatus, errorThrown);
        }
    });
}
function deleteUser(username) {
    if (confirm(`Вы уверены, что хотите удалить пользователя ${username}?`)) {
        $.ajax({
            url: '../index.php?page=deleteTmzUser',
            method: 'POST',
            data: { username: username },
            dataType: 'json', // Убедитесь, что вы ожидаете JSON
            success: function(response) {
    // Проверяем, что ответ действительно является объектом
    if (response && typeof response === 'object') {
        ...
        alert('Неожиданный ответ от сервера.');
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error('Ошибка удаления пользователя:', textStatus, errorThrown);
...
$(document).ready(function() {
    $('#addUserForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '../index.php?page=addTmzUser', // URL запроса для добавления пользователя
            method: 'POST',
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    loadUsers(); // Перезагрузить пользователей
                    $('#addUserForm')[0].reset(); // Очистить форму
                } else {
                    alert('Ошибка: ' + response.message);
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
                console.error('Ошибка при добавлении пользователя:', textStatus, errorThrown);
            }
        });
    });
});
